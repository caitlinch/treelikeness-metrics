...
generate.random.trees <- function(num_trees, num_taxa, tree_depth, output_filepath = NA){
  ...
  for (i in 1:num_trees){
    ...
    # If tree has n tips, the first n values in ape::node.depth.edgelength()
    # are root to tip distances for the tips
    max_r2t = max(ape::node.depth.edgelength(t)[1:ape::Ntip(t)])
    # Scale tree using maximum root to tip distance
    t$edge.length <- t$edge.length * (tree_depth / max_r2t)
    trees[[i]] <- t
  }
