...
if (run_exp1 == FALSE){
  ...
  exp1_df_name <- paste0(local_directory, "exp1_treelikeness_metrics_collated_results.csv")
...
if (run_exp2 == TRUE){
  ...
  exp2_df_name <- paste0(local_directory, "exp2_treelikeness_metrics_collated_results.csv")
...
for (e in exp_ids){
  ...
  # Open results csv
  e_results_file <- paste0(results_dir, grep("rerun", grep(e, grep("treelikeness_metrics_results", results_files, value = TRUE), value = TRUE), value = TRUE, invert = TRUE))
