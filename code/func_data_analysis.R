...
collate.treelikeness.results <- function(alignment_path, experiment_number){
  # Function to take one alignment, get the parameters csv and treelikeness
  # results csv, glue them together and return the new csv as a data.frame
  ...
  if (file.exists(alignment_params_file) == TRUE & file.exists(alignment_treelikeness_file) == TRUE){
    ...
    if (experiment_number == 1){
      ...
      p_df <- p_df[,
                   c("row_id", "uid" , "num_taxa", "num_trees", "tree_depth",
                     "num_reps", "alisim_gene_models", "alisim_gene_tree_length",
      ...
      p_df <- p_df[,
                   c("row_id", "uid" , "num_taxa", "num_trees",
                     "tree_depth_coalescent", "recombination_value",
                     "recombination_type", "num_reps",
                     "alisim_gene_models", "alisim_gene_tree_length",
                     "total_alignment_length", "sequence_type")
      ]
...
reformat.network.treelikeness.test.results.exp3 <- function(id, params_df, results_df){
  # For processing experiment 3 results
  # Function to collect the proportion of treelike alignments for each set of parameters
  # Reformat network treelikeness test results for pretty plotting
  ...
  # Filter results_df to only rows including the relevent params
  row_results_df <- results_df[(results_df$num_taxa == row$num_taxa &
                                  results_df$tree_age == row$tree_age &
                                  results_df$speciation_rate == row$speciation_rate &
                                  results_df$recombination_value == row$recombination_value &
                                  results_df$recombination_type == row$recombination_type), ]
  # Find number of treelike and non-treelike results
  tl_results <- length(which(row_results_df$NetworkTreelikenessTest == "Treelike"))
...
copy.empirical.alignment <- function(row_id, data_df){
  # Small function to take a row from a dataframe, save a copy of the alignment,
  # and save the row containing the alignment parameters as a csv
...
format.timers <- function(time_csv){
  # A small function to read in a timing csv and read out a nice row of how
  # long each step took
  ...
  test_cols <- c("uid", "likelihood_mapping", "scfs", "ntlt", "delta_plot",
                 "q_residuals", "fast_tiger", "Cunningham_test", "tree_proportion",
...
format.one.time <- function(time_df, start_time_label, end_time_label){
  # Small function to take a start and end label (corresponding to objects
  # within a column) and find the time difference in seconds between them
