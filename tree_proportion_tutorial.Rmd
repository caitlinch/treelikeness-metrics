...
## 2. Generate the sequence alignments
For this tutorial, we will use three toy sequence alignments. These alignments are in the `caitlinch/treelikeness-metrics` repository, at `caitlinch/treelikeness-metrics/tutorial/`.
...
* `alignment2.fasta` is 1000 bp long with 5 taxa. This alignment was constructed by concatenating 2 simulated DNA alignments, each generated along a separate random phylogenetic tree.
* `alignment3.fasta` is 1000 bp long with 10 taxa. This alignment was constructed by concatenating 10 simulated DNA alignments,  each generated along a separate random phylogenetic tree.

The code below is to generate these alignments. If you want to generate these alignments, set the flag below to `generate.alignments = TRUE`

```{r}
# Flag to generate alignments
...
**4.c.** Calculate the tree proportion for alignment 3
```{r}
...
**4.a.** Applying the parametric bootstrap to alignment 1
```{r}
...
tree.proportion.parametric.bootstrap(alignment, paste0(repo_path, "tutorial/alignment1.fasta"),
                                     output_directory = al1_dir, treefile, 
