...
cunningham.test <- function(
    alignment_path,
    sequence_format = "DNA",
    iqtree2_path,
    iqtree2_number_threads = "AUTO",
    iqtree_substitution_model = "JC",
    distance_matrix_substitution_model = "JC69",
    base_frequencies = NA,
    Q_matrix = NA,
    number_of_rate_categories = NA
){
  ...
  dna_mat <- calculate.dna.pairwise.distance.matrix(
    alignment_path,
    sequence_format,
    distance_matrix_substitution_model,
    base_frequencies,
    Q_matrix,
    number_of_rate_categories
  )
  ...
  if (file.exists(paste0(alignment_path, ".treefile")) == FALSE){
    call.iqtree2(
      ...
      safe_flag = FALSE,
      bootstraps = NA,
      model = iqtree_substitution_model)
  }
  # Open the tree
  ...
  # Now reorder the t_mat so the taxa are in the same order (and remove spaces)
  dna_order <- gsub(" ", "", attr(dna_mat, "Labels"))
...
treelikeness.metrics.simulations <- function(
    alignment_path,
    iqtree2_path,
    splitstree_path,
    phylogemetric_path,
  ...
  if (file.exists(df_name) == TRUE & redo == FALSE){
    ...
    lm <- likelihood.mapping(
      alignment_path,
      iqtree2_path,
      iqtree2_number_threads = num_iqtree2_threads,
    ...
    scfl_op <- scfl(
      ...
      iqtree2_path,
      iqtree2_number_threads = num_iqtree2_threads,
      number_scf_quartets = num_iqtree2_scf_quartets,
      substitution_model = iqtree_substitution_model
    )

    # Apply Network Treelikeness Test (Huson and Bryant 2006)
    ntlt <- network.treelikeness.test(
      alignment_path,
      splitstree_path,
      sequence_format = sequence_format,
    ...
    mean_delta_plot_value <- mean.delta.plot.value(
      alignment_path,
      sequence_format = sequence_format,
      substitution_model = distance_matrix_substitution_method,
      base_frequencies = NA,
      Q_matrix = NA,
      number_of_rate_categories = NA
    )
    ...
    q_residual_results <- q_residuals(
      alignment_path,
      phylogemetric_path,
      sequence_format = sequence_format,
      phylogemetric_number_of_threads = num_phylogemetric_threads
    )
    mean_q_residual <- q_residual_results$mean_q_residual

    if (apply.TIGER == TRUE){
      # Apply TIGER (Cummins and McInerney 2011)
      mean_tiger_value <- TIGER(
        alignment_path,
        fast_TIGER_path,
        sequence_format = sequence_format
      )
    ...
    cunningham_metric <- cunningham.test(
      alignment_path,
      sequence_format,
      iqtree2_path,
      iqtree2_number_threads = num_iqtree2_threads,
    ...
    tree_proportion <- tree.proportion.long(
      alignment_path,
      sequence_format = sequence_format,
      model = distance_matrix_substitution_method,
      remove_trivial_splits = tree_proportion_remove_trivial_splits,
      check_iqtree_log_for_identical_sequences = FALSE,
      run_splitstree = run_splitstree_for_tree_proportion,
    ...
    results_vec <- c(
      ...
      scfl_op$mean_scf,
      scfl_op$median_scf,
      min(scfl_op$all_scfs),
      max(scfl_op$all_scfs),
      ntlt,
    ...
    results_df <- as.data.frame(matrix(
      data = results_vec,
      nrow = 1,
      ncol = length(results_vec),
      byrow = TRUE
    ))
    names_vec <- c(
      "LM_num_resolved_quartets",
      "LM_num_partly_resolved_quartets",
      "LM_num_unresolved_quartets",
      "LM_total_num_quartets",
